...
const EVENTS = [
  ...
  'Added to Store Inventory',
  'Ready for Sale',
  'Sold/Delivered to User',
];

export default function RetailerEventScreen() {
  ...
  const loadBatches = async () => {
    try {
      ...
      console.log('LOAD RETAILER ERROR:', error);
  ...
  const pendingBatches = useMemo(() => {
    return batches.filter((item) => item.currentActor !== 'retailer');
  }, [batches]);
  ...
  const updateBatches = useMemo(() => {
    return batches.filter(
      (item) =>
        item.currentActor === 'retailer' &&
        item.lastEventType !== 'Ready for Sale' &&
        item.lastEventType !== 'Sold/Delivered to User'
    );
  }, [batches]);
  ...
  const currentList = useMemo(() => {
    ...
    if (selectedTab === 'ready') return readyBatches;
    return pendingBatches;
  }, [selectedTab, pendingBatches, updateBatches, readyBatches]);
  ...
  const saveEvent = async (batch) => {
    try {
      ...
      await updateDoc(doc(db, 'batches', batchId), {
        ...
        currentActor: 'retailer',
        updatedAt: serverTimestamp(),
      });

      Alert.alert('Saved', 'Retailer event added successfully');
  ...
  const renderEmpty = () => {
    ...
    if (selectedTab === 'ready') return 'No ready for sale batches';
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSub}>Manage batches received from distributor</Text>
      </View>
      ...
      <View style={styles.tabsWrapper}>
        ...
        <TouchableOpacity
          style={[styles.tabButton, selectedTab === 'ready' && styles.activeTab]}
        ...
        <ScrollView contentContainerStyle={styles.listContent}>
          {currentList.length === 0 ? (
            ...
            currentList.map((batch) => {
              const batchEvent =
                selectedEvent[batch.id] || 'Received from Distributor';
                  ...
                  {selectedTab !== 'ready' && (
                    <>
                      ...
                      <View style={styles.readOnly}>
                        <Text style={styles.readOnlyText}>Distributor</Text>
                      </View>
